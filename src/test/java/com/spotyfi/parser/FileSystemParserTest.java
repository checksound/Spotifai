...
class FileSystemParserTest {
	...
	void testGetFilesInDirectory() {
		...
		assertFalse(filesInDirectory.contains(pathFileVadoInMessico));
		assertTrue(filesInDirectory.contains(pathFileCosaSuccede));
		
	}
	
	@Test
	void testGetFilesWithoutExtensionInDirectory() {
		...
		List<Path> filesInDirectory = fsp.getFilesInDirectory(pDir, null);
		
		Path pathFileBollicine = Paths.get(pathDir, "Bollicine.txt");
		Path pathFileCosaSuccede = Paths.get(pathDir, "Cosa_succede_in_citta.txt");
		Path pathFilePDF = Paths.get(pathDir, "file.pdf");
		
		assertEquals(3, filesInDirectory.size());
		
		assertTrue(filesInDirectory.contains(pathFileBollicine));
		assertTrue(filesInDirectory.contains(pathFilePDF));
		assertTrue(filesInDirectory.contains(pathFileCosaSuccede));
